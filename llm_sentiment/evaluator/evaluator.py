...
class Evaluator:
    def __init__(
        ...
        user_prompt_template: str = "Please analyze the following Amazon product review and rate it on a scale from 1 to 5 stars, where 1 is the most negative and 5 is the most positive. Provide ONLY the numerical rating (1, 2, 3, 4, or 5) without any explanation.\n\nReview: {review}"
    ):
    ...
    async def evaluate_single(self, review_text: str, actual_rating: Optional[int] = None) -> Dict[str, Any]:
        ...
        result = {
            'review_text': review_text,
            'system_prompt': self.system_prompt,
            'user_prompt': self.user_prompt_template.format(review=review_text),
    ...
    def calculate_metrics(self, results: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        for i, rating in enumerate([1, 2, 3, 4, 5]):
            metrics[f'precision_rating_{rating}'] = precision[i] if i < len(precision) else 0.0
            metrics[f'recall_rating_{rating}'] = recall[i] if i < len(recall) else 0.0
            metrics[f'f1_rating_{rating}'] = f1[i] if i < len(f1) else 0.0
